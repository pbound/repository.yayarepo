...
def add_item( action="" , title="" , plot="" , url="" ,thumbnail="" , extra="",mmsId="",page="",token="",epid="",gtitle="", yid="" , isPlayable = False, folder=True ):
    ...
    if url.startswith("plugin://"):
        ...
        listitem.setProperty('IsPlayable', 'true')
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&yid=%s' % ( sys.argv[ 0 ] , action , urllib.quote_plus( title ) , urllib.quote_plus(url) , urllib.quote_plus( thumbnail ) , urllib.quote_plus( plot ) , urllib.quote_plus( extra ), urllib.quote_plus( yid ) )
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
    else:
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s&yid=%s&gtitle=%s' % ( sys.argv[ 0 ] , action , urllib.quote_plus( title.encode('utf-8') ) ,
                                                                                                         urllib.quote_plus(url.encode('utf-8')) , urllib.quote_plus( thumbnail.encode('utf-8') )
                                                                                                         , urllib.quote_plus( plot ) , urllib.quote_plus( extra.encode('utf-8') ), urllib.quote_plus(page ),
                                                                                                         urllib.quote_plus( yid ),urllib.quote_plus( gtitle.encode('utf-8')))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
