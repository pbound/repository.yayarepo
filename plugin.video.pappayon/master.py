...
def nungsub(url):
    ...
    if url is not None:
        ...
        for frm in opt:
            ...
            r = requests.get(surl)
            r = HTMLParser.HTMLParser().unescape(r).text
            strm = re.compile('<iframe.*?src="(http+[^"]+)"').findall(r)
            if len(strm)>0:
...
def m2f(url):
    # print url
    frameurl = getitems(url,'style.*\s.*?<iframe.src="([^"]+)"')
    # print frameurl

    headers = {'User-Agent': 'Mozilla/5.0 (X11; Linux i686; rv:45.0) Gecko/20100101 Firefox/45.0',
               'Referer': 'https://www.movie2free.com/'}
    r = requests.get(frameurl, headers=headers).text
    r = HTMLParser.HTMLParser().unescape(r)
    # r = r.decode('unicode_escape').encode('utf-8').replace('\/', '/')
    r = r.decode('unicode_escape').replace('\/', '/')
    # print r
    streamf = re.compile('label"."(.*?)".*?url":"([^"]+)"').findall(r)
    # print len(streamf)

    for i in range(0,len(streamf)):
            # strm = src.get('data-lazy-src')
        strmlist.append({"url": streamf[i][1], "title": 'movie2free  >>'+ streamf[i][0]})
    return strmlist
...
def fidmov(url):
    ...
    if 'kod-hd' in url:
        Kodhd(getitems(url,'class="moviefilm.*?href="([^"]+)"'))
        return strmlist
    elif '037hd' in url:
        hd037(getitems(url, 'class="moviefilm.*\n.*?href="([^"]+)"'))
        return strmlist
    elif 'www.mastermovie-hd.com' in url:
        mastermov(getitems(url,' class="item-content.*\n.*\n.*?href="([^"]+)'))
        return strmlist
    # elif 'nanamovies' in url:
    #     nana(getitems(url,'class="film-list-playbtn.*?href="([^"]+)"'))
    #     return strmlist
    elif 'nungsub.com' in url:
        nungsub(getitems(url,'class="item-conten.*?href="([^"]+)"'))
        return strmlist
    elif '2youhd.com' in url:
        youhd(getitems(url, 'class="moviefilm.*\n.*?href="([^"]+)"'))
        return strmlist
    elif '9nung.com' in url:
        nung9(getitems(url,'class="thumb.*\n.*?href="([^"]+)"'))
        return strmlist
    elif 'vojkud.com' in url:
        vojkud(getitems(url,'class="boxinfo.*\n.*?href="([^"]+)"'))
        return strmlist
    # elif 'movie2free.com' in url:
    #     m2f(getitems(url,'movie-title.*\n.a.href="([^"]+)'))
    #     return strmlist
    return strmlist
...
# url = 'https://nungsub.com/?s=batman-and-harley'
# print fidmov(url)
# url = 'https://kod-hd.com/2018/01/29/mother-2017-%e0%b8%a1%e0%b8%b2%e0%b8%a3%e0%b8%94%e0%b8%b2-hd/'
# url = 'https://www.movie2free.com/?s=colossal'
# url = 'https://www.037hd.com/?s=spectre'
# url = 'https://www.037hd.com/?s=taxi'
# url = 'https://www.037hd.com/maze-runner-death-cure-2018-%E0%B9%80%E0%B8%A1%E0%B8%8B-%E0%B8%A3%E0%B8%B1%E0%B8%99%E0%B9%80%E0%B8%99%E0%B8%AD%E0%B8%A3%E0%B9%8C-%E0%B9%84%E0%B8%82%E0%B9%89%E0%B8%A1%E0%B8%A3%E0%B8%93%E0%B8%B0/'
# url = 'https://www.037hd.com/foreigner-2017-2-%e0%b9%82%e0%b8%84%e0%b8%95%e0%b8%a3%e0%b8%9e%e0%b8%a2%e0%b8%b1%e0%b8%84%e0%b8%86%e0%b9%8c%e0%b8%9c%e0%b8%b9%e0%b9%89%e0%b8%a2%e0%b8%b4%e0%b9%88%e0%b8%87%e0%b9%83%e0%b8%ab/"'
...
# print youhd(url)
# print m2f(getitems(url,'movie-title.*\n.a.href="([^"]+)'))
# print m2f('https://www.movie2free.com/black-panther-%E0%B9%81%E0%B8%9A%E0%B8%A5%E0%B9%87%E0%B8%84-%E0%B9%81%E0%B8%9E%E0%B8%99%E0%B9%80%E0%B8%98%E0%B8%AD%E0%B8%A3%E0%B9%8C-2018/')