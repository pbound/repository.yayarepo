...
def getSearch(url, qry):
    if qry is None: exit()
    # pt = sys.path[0] + '/phantomjs'
    # dr = webdriver.PhantomJS(executable_path=pt)
    # url = 'https://www.viu.com/ott/th/th?r=search'
    dr.get(url)
    element = dr.find_element_by_id('search')
    element.clear()
    element.send_keys(qry)
    ...
    soup = BeautifulSoup(html, 'html5lib')
    soup.prettify()
    ul = soup.find('ul', {"id": "search_list_series"})
    # print ul
    div = ul.findAll('div', {"class": "grid "})
    # print div.get('onclick')
    # print (div)
    seriesList = []
    for item in div:
        # print item.find('a')
        title = item.find('div', {'class': 'figure-title'}).text
        stg = item.find('a').get('onclick')
        # print stg.find(',')
        url = '/ott/th/th/vod/' + (stg[8:stg.find(',')]) + '/'
        seriesList.append({'title': title, 'url': url,
...
def getEpisodes(url):
    ...
    for ep in li:
        ...
        episodesList.append({"title": title, "url": ep.find('a').get('href'),
                             'thumbnail': ep.find('img').get('data-original').replace(
                                 '/ott/th/v1/imgprocess/reduceImage.php?p=30&img=', ''), 'epid': id})
    return episodesList
...
def get_sub(url):
    gurl = 'https://www.grabvdo.com/?url=https://www.viu.com' + url
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0',
               'Referer': 'http://www.kseries.co/'}
    source = requests.get(gurl, headers=headers).text
    source = source.encode('unicode_escape')  # .encode('utf8')
    # print source
    sub_qry = re.compile('u0e20.*?href="([^"]+)"').findall(source)
    surl = sub_qry[0]

    downloadsub(surl)


def get_sub_backup(url):
...
def extractLinks(a):
    ...
    # print get_sub('/ott/th/th/vod/67440/Hate-to-Love-You')
