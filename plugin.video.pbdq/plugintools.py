...
def add_item( action="" , title="" , plot="" , url="" ,thumbnail="" , extra="",mmsId="",page="",token="",epid="",gid="", yid="" , isPlayable = False, folder=True ):
    ...
    if url.startswith("plugin://"):
        ...
        itemurl = '%s?action=%s&title=%s&url=%s&thumbnail=%s&plot=%s&extra=%s&page=%s&yid=%s&gid=%s' % ( sys.argv[ 0 ] , action , urllib.quote_plus( title.encode('utf-8') ) , urllib.quote_plus(url.encode('utf-8')) , urllib.quote_plus( thumbnail.encode('utf-8') ) , urllib.quote_plus( plot ) , urllib.quote_plus( extra ), urllib.quote_plus(page ),urllib.quote_plus( yid ),urllib.quote_plus( gid ))
        xbmcplugin.addDirectoryItem( handle=int(sys.argv[1]), url=itemurl, listitem=listitem, isFolder=folder)
